...
## 7. 使用 `YitIdHelper` 的雪花 ID 方案（替代 `LocalId/ServerId`）

为减少双 ID 映射复杂度与回写失配风险，建议将本地创建实体统一改为使用 `YitIdHelper` 生成雪花 ID，并推动服务端同步链路兼容同一 ID 语义。

### 7.1 设计目标

- 用单一 ID 语义替代 `LocalId/ServerId` 双轨映射；
- 本地离线创建即可获得全局唯一 ID，避免多端并发创建时主键碰撞；
- 保持 MVP 目标不变：先保证“可上传、可回写、可观测”。

### 7.2 方案要点

- 统一引入 `YitIdHelper` 生成 64 位雪花 ID（`long`）；
- `Material`、`Provider`、`SyncState.EntityId` 在客户端统一使用雪花 ID；
- 上行 Create 时传递该 ID（作为业务实体 ID 或 `clientGeneratedId`）；
- 服务端回包与下行变更优先按雪花 ID 对应，不再依赖 `LocalId -> ServerId` 映射。

### 7.3 最小落地策略（分阶段）

1. **阶段 A（MVP优先）**：  
   客户端接入 `YitIdHelper`，新增 `clientGeneratedId` 字段参与请求/回写关联；服务端主键暂不替换。

2. **阶段 B（一致性增强）**：  
   服务端支持按 `clientGeneratedId` 幂等落库与回包，消除 Create 场景按顺序关联的脆弱性。

3. **阶段 C（统一主键）**：  
   评估双端主键迁移到 `bigint/long`，实现端到端单 ID 体系。

### 7.4 上传链路约定（Create）

1. 本地新建实体时即生成雪花 ID，并进入 `Pending`。  
2. Create 请求携带 `clientGeneratedId`（或直接实体主键为雪花 ID）。  
3. 回包处理按 `clientGeneratedId` 稳定定位 `SyncState`，标记 `Applied`，避免 `entityId` 反查失效。

### 7.5 下行链路约定

- 下行数据优先按雪花 ID 命中本地实体；
- 若未命中，再走插入逻辑；
- 禁止在同步主流程中假设“本地自增 ID == 服务端自增 ID”。

### 7.6 实施边界与风险

- MVP 阶段可不一次性改造全部表结构；优先保证请求关联稳定；
- 雪花 ID 依赖时钟与节点配置，需明确 `workerId` 分配与回拨处理策略；
- 重复业务数据（同名同证号）仍需唯一约束或运营流程处理，雪花 ID 不负责业务去重。
