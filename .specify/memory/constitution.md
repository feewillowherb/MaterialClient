...
- **Struct 优先原则（低分配优化）**：尽量使用 `struct` 解决低分配问题，减少堆内存分配和 GC 压力。
  - **适用场景**：对于小的、不可变的值类型数据，优先使用 `struct` 而非 `class`，例如坐标点、配置选项、状态标志等。
  - **Struct 设计原则**：
    - Struct 应该是不可变的（immutable），使用 `readonly struct` 确保值不会被意外修改。
    - Struct 大小应控制在合理范围内（通常建议小于 16-24 字节），避免过大的 struct 导致性能下降。
    - 对于需要引用语义或较大数据结构的场景，仍应使用 `class`。
  - **性能优势**：
    - Struct 分配在栈上，避免堆分配和 GC 压力。
    - 值类型传递时进行值拷贝，避免引用追踪开销。
    - 适合高频调用、热路径代码的性能优化。
  - **注意事项**：
    - 避免在 struct 中包含引用类型字段（除非是 `readonly` 字段），防止意外的堆分配。
    - 使用 `readonly struct` 和 `in` 参数修饰符优化大 struct 的传递性能。
    - 对于需要多态或继承的场景，应使用 `class` 而非 `struct`。
...
- **实施计划语言（NON-NEGOTIABLE）**：
  - 实施计划文档（如 plan.md、`.cursor/plans/*.plan.md`）必须使用英文撰写，禁止在计划正文中使用中文。
...
**Version**: 1.4.1 | **Ratified**: 2025-01-27 | **Last Amended**: 2025-02-01
