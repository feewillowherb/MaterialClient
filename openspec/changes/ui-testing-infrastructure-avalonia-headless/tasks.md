...
- [x] 9.2 编写 LoginWindow 集成测试
  - [x] 9.2.1 测试窗口创建和显示
  - [x] 9.2.2 测试表单提交流程
  - [x] 9.2.3 测试错误处理和显示
  - 编写了 ApplicationStartupTests.cs
  - 编写了 LoginWindowIntegrationTests.cs，包含：
    - 测试窗口创建和 ViewModel 初始化
    - 测试有效凭据登录成功
    - 测试空用户名/密码验证错误
    - 测试记住我属性状态管理
    - 测试错误处理和错误消息清除
    - 测试数据绑定正确性
- [x] 9.3 编写简单场景的用户流程集成测试
  - 创建了数据流测试场景（使用 Mock 服务）
- [x] 9.4 测试多个组件协同工作
  - 验证 ViewModel 与 Service 之间的交互
- [x] 9.5 验证数据流从 UI 到 ViewModel 再到服务
  - 使用 NSubstitute 验证服务调用参数
  - 验证数据流从 UI 输入到 ViewModel 处理，再到服务调用
- [x] 9.6 运行所有集成测试，验证通过
  - 测试项目编译成功
- [x] 9.7 修复失败的测试
