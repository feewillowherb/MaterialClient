...
**Change ID**: `unified-lpr-data-transfer-and-capture-interface`
**Type**: MODIFIED
**Date**: 2026-01-29

This document specifies the requirements modified by the `unified-lpr-data-transfer-and-capture-interface` change. Only the requirements that have changed are included here. Refer to the base specification for the complete set of requirements.
...
系统 SHALL使用 ReactiveUI MessageBus 从硬件回调处理程序向业务服务交付车牌识别事件,解耦硬件集成层和业务逻辑层。

#### Scenario: 海康威视设备识别车牌并发布 MessageBus 消息
- **GIVEN** 系统配置了海康威视 LPR 设备
- **AND** 海康威视 SDK 回调 `MSGCallBack` 接收到 `COMM_UPLOAD_PLATE_RESULT` (0x2800) 消息
- **AND** `MinimalWebHostService` 中的回调处理程序被调用
- **WHEN** 回调解析车牌识别结果时
- **THEN** 系统 SHALL:
...
- **GIVEN** 系统配置了 LprAllInOne 设备
- **AND** HTTP 回调端点接收到带有 `type=online` 的 POST 请求
- **AND** 表单数据包含 `plate_num` 参数
- **WHEN** `MinimalWebHostService` 中的回调处理程序处理请求时
- **THEN** 系统 SHALL:
  - 创建包含以下内容的 `LicensePlateRecognizedMessage`:
...
- **GIVEN** 系统配置了华夏智信设备
- **AND** HTTP 回调端点接收到带有识别车牌数据的 POST 请求
- **WHEN** `MinimalWebHostService` 中的回调处理程序处理请求时
- **THEN** 系统 SHALL:
  - 创建包含以下内容的 `LicensePlateRecognizedMessage`:
...
系统 SHALL配置 `AttendedWeighingService` 订阅通过 ReactiveUI MessageBus 的 `LicensePlateRecognizedMessage`,通过现有的车牌缓存和推荐逻辑处理识别事件。

#### Scenario: AttendedWeighingService 在初始化时订阅 LPR 消息
- **GIVEN** `AttendedWeighingService` 正作为单例依赖被实例化
- **WHEN** 执行服务构造函数时
- **THEN** 系统 SHALL:
...
- **GIVEN** `AttendedWeighingService` 具有活动的 MessageBus 订阅
- **AND** `LicensePlateRecognizedMessage` 被发布到总线
- **WHEN** 订阅处理程序接收消息时
- **THEN** 系统 SHALL:
  - 记录包含设备信息的识别事件
...
- **GIVEN** `AttendedWeighingService` 正在被释放
- **AND** MessageBus 订阅处于活动状态
- **WHEN** 调用 `DisposeAsync()` 方法时
- **THEN** 系统 SHALL:
...
系统 SHALL提供统一的 `ILprDevice` 接口,定义所有 LPR 设备类型的主动抓拍能力,包括海康威视、LprAllInOne 和华夏智信设备。

#### Scenario: ILprDevice 接口定义主动抓拍能力
- **GIVEN** 系统需要统一的主动抓拍接口
- **WHEN** 定义 `ILprDevice` 接口时
- **THEN** 接口应包含:
...
- **GIVEN** 系统配置了海康威视 LPR 设备
- **AND** `HikvisionLprService` 实现 `ILprDevice` 接口
- **AND** `SupportsActiveCapture` 返回 `true`
- **WHEN** 应用调用 `TriggerCaptureAsync(config)` 时
- **THEN** 系统 SHALL:
...
- **GIVEN** 系统配置了 LprAllInOne 设备
- **AND** `LprAllInOneService` 实现 `ILprDevice` 接口
- **AND** `SupportsActiveCapture` 返回 `true`
- **WHEN** 应用调用 `TriggerCaptureAsync(config)` 时
- **THEN** 系统 SHALL:
...
- **GIVEN** 系统配置了华夏智信设备
- **AND** `HuaxiazhixinLprService` 实现 `ILprDevice` 接口
- **AND** `SupportsActiveCapture` 返回 `false`
- **WHEN** 应用调用 `TriggerCaptureAsync(config)` 时
- **THEN** 系统 SHALL:
  - 记录警告日志,说明厂商不支持主动抓拍
...
系统 SHALL提供统一的消息类,用于通过 ReactiveUI MessageBus 传输车牌识别数据,支持所有 LPR 设备类型且结构一致。

#### Scenario: LicensePlateRecognizedMessage 携带完整识别数据
- **GIVEN** 来自任何 LPR 设备类型的车牌识别事件发生
- **WHEN** 创建 `LicensePlateRecognizedMessage` 时
- **THEN** 消息应包含:
...
- **GIVEN** 创建了包含完整数据的 `LicensePlateRecognizedMessage`
- **WHEN** 使用 `MessageBus.Current.SendMessage(message)` 发布消息时
- **THEN** 系统 SHALL:
...
系统 SHALL从 `IAttendedWeighingService` 公共接口中移除 `OnPlateNumberRecognized` 方法,使其成为通过 MessageBus 订阅调用的私有实现细节。

#### Scenario: IAttendedWeighingService 接口排除 OnPlateNumberRecognized 方法
- **GIVEN** 定义了 `IAttendedWeighingService` 接口
- **WHEN** 重构完成时
- **THEN** 接口应**不**包含:
...
- **GIVEN** `AttendedWeighingService` 类实现 `IAttendedWeighingService`
- **AND** MessageBus 订阅处于活动状态
- **WHEN** 订阅处理程序接收 `LicensePlateRecognizedMessage` 时
- **THEN** 类应调用私有的 `OnPlateNumberRecognized()` 方法
- **AND** 方法应**不**可从外部代码访问
- **AND** 方法应保留其现有实现逻辑(缓存、过滤等)

---

### Requirement: MessageBus 订阅的内存泄漏预防

系统 SHALL确保所有 MessageBus 订阅正确释放,以防止长期运行场景中的内存泄漏。

#### Scenario: 订阅正确存储和释放
- **GIVEN** 服务创建 MessageBus 订阅
- **WHEN** 创建订阅时
- **THEN** 系统 SHALL:
...
#### Scenario: 服务清理时释放订阅
- **GIVEN** 具有活动 MessageBus 订阅的服务正在被释放
- **WHEN** 调用 `DisposeAsync()` 或 `Dispose()` 方法时
- **THEN** 系统 SHALL:
...
- **GIVEN** 系统运行了长时间(24+ 小时)
- **AND** LPR 设备识别了 1000+ 个车牌
- **WHEN** 监控内存使用时
- **THEN** 系统 SHALL:
...
系统 SHALL正确处理主动抓拍操作中的错误情况,包括网络超时、设备离线和 SDK 调用失败,并在合理的时间内超时。

#### Scenario: 主动抓拍在设备离线时超时
- **GIVEN** 设备配置为离线或不可达
- **AND** 应用调用 `TriggerCaptureAsync(config)`
- **WHEN** 登录尝试超时时
- **THEN** 系统 SHALL:
...
- **GIVEN** 设备在线但 SDK 调用失败
- **AND** 应用调用 `TriggerCaptureAsync(config)`
- **WHEN** `NET_DVR_ContinuousShoot()` 或其他 SDK 调用返回 false
- **THEN** 系统 SHALL:
...
系统 SHALL在主动抓拍操作后正确清理所有资源,包括设备登出、订阅释放和内存清理。

#### Scenario: 主动抓拍操作后清理资源
- **GIVEN** 主动抓拍操作已完成(成功或失败)
- **WHEN** 可观察流完成或出错时
- **THEN** 系统 SHALL:
...
- **GIVEN** 主动抓拍操作正在进行
- **AND** 订阅者取消订阅(调用返回的 `IDisposable.Dispose()`)
- **WHEN** 取消发生时
- **THEN** 系统 SHALL:
  - 登出设备(如果已登录)
  - 释放所有内部订阅
