...
- [ ] 1.1 Create `SessionRefreshRequiredEto` class in `MaterialClient.Common/Events/`
...
- [ ] 2.1 Modify `MaterialPlatformBearerTokenHandler.SendAsync` to detect 401 responses
...
- [ ] 3.1 Locate `PollingBackgroundService` implementation
  - Find where background workers are registered in `MaterialClientModule`

- [ ] 3.2 Add `SessionRefreshRequiredEto` subscription to polling service
...
- [ ] 3.3 Implement re-login logic in event handler
...
- [ ] 5.1 Update AGENTS.md if new patterns are established
