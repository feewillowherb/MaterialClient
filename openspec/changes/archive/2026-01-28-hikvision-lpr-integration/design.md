...
#### 1.1 LicensePlateRecognitionConfig Entity
...
#### 2.1 SettingsWindowViewModel Extension
...
#### 2.2 LicensePlateRecognitionConfigViewModel Extension
...
#### 2.3 AddLprDialogViewModel Extension
...
**SettingsWindow.axaml - Hikvision Fields**:
```xml
<!-- 车牌识别配置列表 -->
...
#### 3.1 IHikvisionLprService Interface
...
**Event Type Definition (Temporary)**:
```csharp
...
**Implementation Note**:
本提案仅定义接口,具体实现(包括 HCNetSDK 集成、Rx 订阅管理、内存泄漏预防等)在后续提案中完成。
...
**Impact**: 低 - 配置数据量小(通常 < 10 条记录)

**Optimization**: 不需要优化

**Note**: `System.Text.Json` 性能优异,对于小配置列表序列化/反序列化耗时 < 1ms。

### UI Rendering Performance

**Impact**: 低 - 条件显示仅影响少量控件(4 个字段)

**Optimization**: 不需要优化

**Potential Issue**:
- 频繁切换 `LprDeviceType` 可能导致重复布局计算
- **Mitigation**: 使用 `IsVisible` 而非 `Visibility` 属性,避免布局重建

### Memory Usage

**Impact**: 可忽略 - 每个配置对象增加 ~100 字节(4 个字符串引用)
...
### Out of Scope (Requires Separate Proposal)
