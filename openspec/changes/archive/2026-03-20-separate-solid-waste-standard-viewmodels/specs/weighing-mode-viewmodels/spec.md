...
### Requirement: 派生类必须实现抽象方法

系统必须要求派生类实现 `SaveCoreAsync()` 和 `CompleteCoreAsync()` 抽象方法，以处理模式特定的保存和完成逻辑。

#### Scenario: 标准模式保存实现
- **WHEN** 调用 StandardModeDetailViewModel.SaveCoreAsync()
- **THEN** 系统必须调用 `IWeighingMatchingService.UpdateListItemAsync()`
- **AND** 使用标准模式的参数格式

#### Scenario: 固废模式保存实现
- **WHEN** 调用 SolidWasteModeDetailViewModel.SaveCoreAsync()
- **THEN** 系统必须调用 `IWeighingMatchingService.UpdateSolidWasteModeAsync()`
- **AND** 包含固废模式特有参数（联单号、镇街、类型）

#### Scenario: 标准模式完成实现
- **WHEN** 调用 StandardModeDetailViewModel.CompleteCoreAsync()
- **THEN** 系统必须验证供应商、物料、物料单位、运单数量
- **AND** 调用 `IWeighingMatchingService.CompleteOrderAsync()`

#### Scenario: 固废模式完成实现
- **WHEN** 调用 SolidWasteModeDetailViewModel.CompleteCoreAsync()
- **THEN** 系统必须验证供应商、材料、镇街、类型、联单号
- **AND** 调用 `IWeighingMatchingService.CompleteOrderAsync()`

### Requirement: ViewModel 必须支持依赖注入
...
#### Scenario: 通过 DI 获取 ViewModel
- **WHEN** 调用 `_serviceProvider.GetRequiredService<StandardModeDetailViewModel>()`
- **THEN** 系统必须返回一个新的 StandardModeDetailViewModel 实例
- **AND** 所有依赖项必须正确注入

### Requirement: ViewModel 必须支持 ReactiveUI 模式

系统必须使用 ReactiveUI 和 ReactiveUI.SourceGenerators 实现属性和命令。

#### Scenario: 使用 Reactive 属性
- **WHEN** 定义 ViewModel 属性
- **THEN** 系统必须使用 `[Reactive]` 特性标记可变属性
- **AND** 使用 `ReactiveUI.SourceGenerators` 自动生成代码

#### Scenario: 使用 ReactiveCommand
- **WHEN** 定义 ViewModel 命令
- **THEN** 系统必须使用 `[ReactiveCommand]` 特性标记命令方法
- **AND** 支持异步执行

### Requirement: 标准模式 ViewModel 必须提供物料管理功能

系统必须通过 StandardModeDetailViewModel 提供物料选择和管理功能。

#### Scenario: 标准模式物料属性
- **WHEN** 使用 StandardModeDetailViewModel
- **THEN** 系统必须提供以下属性：
  - Providers：供应商列表
...
### Requirement: 固废模式 ViewModel 必须提供固废特有功能

系统必须通过 SolidWasteModeDetailViewModel 提供固废模式特有的功能。

#### Scenario: 固废模式特有属性
- **WHEN** 使用 SolidWasteModeDetailViewModel
- **THEN** 系统必须提供以下属性：
