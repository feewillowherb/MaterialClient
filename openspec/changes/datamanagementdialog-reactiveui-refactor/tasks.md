...
- [x] 1.6 在 ViewModel 中增加导出相关命令或方法（例如 `ExportAsync` 或 `ExportCommand`），接收导出路径参数，内部负责构造 `SolidWasteExportFilter` 并调用 `IExcelExportService.ExportSolidWasteAsync`，返回导出结果 DTO（包含成功标志、行数、错误信息等）
...
- [x] 2.5 更新导出按钮点击事件 `OnExportClick`，使其只负责弹出目录选择对话框并将选定的导出路径传递给 ViewModel 的导出命令/方法，然后根据命令返回结果决定展示成功或失败通知，不在 View 中构造过滤器或直接调用导出服务
...
- [x] 3.4 回归验证导出按钮行为，确保导出流程通过 ViewModel 的导出命令/方法完成（包括过滤器构造和服务调用），View 仅负责选择保存路径和展示通知
