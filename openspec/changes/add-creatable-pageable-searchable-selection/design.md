...
- 需求：单一控件内聚「可搜索、可分页、可创建」能力，关闭状态下与现有 SearchableSelectionBox 视觉一致（高度 32、白底、边框 #E5E7EB、字体 12、前景 #333333、右侧下拉箭头等），与 ViewModel 通过标准属性/委托对接，不依赖具体 VM 类型。
...
- 实现一个 TemplatedControl，内嵌 PART_TextBox + PART_Popup（列表 + 分页 + 可选“新增”），通过 LoadPageAsync、SelectedItem、DisplayMemberPath、GetItemId、Watermark、PageSize 等与调用方对接。
...
- **样式一致性**：新控件需与 SearchableSelectionBox 关闭状态像素级一致。缓解：复用同一组样式资源（Border、字体、颜色、箭头），必要时从现有控件抽取共享 Style。

## Migration Plan

1. **实现阶段**：在 MaterialClient.UI 中新增控件、SelectionItem 与扩展方法；服务层分页接口增加 selectedIds 与过滤逻辑；为新控件编写单元/集成测试。
