...
**Agent 禁止直接修改任何源代码文件。** 所有代码变更必须通过 OpenSpec 工作流驱动。
...
```
用户需求 → Agent 产出 Spec/Design → 人工审批 → 人工实施代码 → Agent 验证一致性
              ▲                                                    │
              └────────────── 反馈循环 ◄────────────────────────────┘
```
...
4. **实施阶段**：**由人工完成代码编写**，Agent 仅可协助阅读和解释 Spec
5. **归档阶段** (`/opsx:archive`)：变更完成后更新 specs，归档 change

### 当用户要求修改代码时

如果用户直接要求 Agent 编写或修改代码，Agent 应：
