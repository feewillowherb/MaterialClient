...
NOTES: Legacy documentation. Preserved for historical reference.
-->
...
3.必须完成AuthCode正确验证后，才能进入登录页面，否则始终让用户输入授权码，知道验证成功位置。并将LicenseInfo写入到数据库。
4.如果数据库中LicenseInfo.AuthEndTime有效。则可以跳过AuthCode，直接进入登录页面。
5.用户登录页面，"记住密码"也要持久化到数据库中,下次登录时自动输入用户名和密码


AuthCode参考页面在/assets/AuthCode.xaml

Login参考页面在/assets/Login.xaml
