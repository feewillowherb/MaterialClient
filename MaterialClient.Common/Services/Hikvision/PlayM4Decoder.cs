...
public sealed class PlayM4Decoder : IDisposable
{
	...
	public bool GetPictureQuality()
	{
		lock (_lockObject)
		{
			...
			if (PlayM4.PlayM4_GetPictureQuality(_port, ref bHighQuality))
			{
				return bHighQuality;
			}
			return false;
	...
	public bool SetPictureQuality(bool highQuality)
	{
		lock (_lockObject)
		{
			if (_port < 0)
	...
	public bool InputData(IntPtr data, uint dataSize)
	{
		lock (_lockObject)
		{
			if (!_isPlaying || _port < 0 || dataSize == 0)
			{
				return false;
			}
	...
	public bool CaptureJpeg(string savePath)
	{
		lock (_lockObject)
		{
			...
			try
			{
				uint jpegSize = 0;
				// 获取 JPEG 数据
				if (!PlayM4.PlayM4_GetJPEG(_port, buffer, (uint)bufferSize, ref jpegSize))
				{
					return false;
				}
				...
				// 写入文件
				System.IO.File.WriteAllBytes(savePath, jpegData);
				return true;
			}
			finally
			{
...
internal static class PlayM4
{
	...
	/// </summary>
	/// <param name="nPort">端口号</param>
	/// <param name="bHighQuality">true 表示高质量，false 表示普通质量</param>
	/// <returns>是否成功</returns>
	[DllImport(DllName)]
